...
function deployCC() {
  scripts/deployCC.sh "${CHANNEL_NAME}" "${CC_NAME}" "${CC_SRC_PATH}" "${CC_SRC_LANGUAGE}" "${CC_VERSION}" "${CC_SEQUENCE}" "${CC_INIT_FCN:-NA}" "${CC_END_POLICY:-NA}" "${CC_COLL_CONFIG:-NA}" "${CLI_DELAY:-3}" "${MAX_RETRY:-5}" "${VERBOSE:-false}"
...
function deployCCAAS() {
  scripts/deployCCAAS.sh "${CHANNEL_NAME}" "${CC_NAME}" "${CC_SRC_PATH}" "${CCAAS_DOCKER_RUN:-true}" "${CC_VERSION}" "${CC_SEQUENCE}" "${CC_INIT_FCN:-NA}" "${CC_END_POLICY:-NA}" "${CC_COLL_CONFIG:-NA}" "${CLI_DELAY:-3}" "${MAX_RETRY:-5}" "${VERBOSE:-false}" "${CCAAS_DOCKER_RUN:-true}"
...
function listChaincode() {
  ...
  # Ensure retry parameters are set for cc helpers
  : ${DELAY:=${CLI_DELAY:-3}}
